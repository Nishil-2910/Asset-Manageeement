...
adapted by jokin: 
    added url parameter to work with another urls
    set don't put the key if the value is empty
...
jQuery.jqURL = {
	...
	function(args) {
	    ...
	    // if no keys passed in, just return url with no querystring
	        ...
	        var qsobj = jQuery.jqURL.qs(args);  // object with key/value pairs		
	        var counter = 0;
	...
	function(hash, args) {
	    ...
	    for (var k in qsobj) {
	        amp = (counter) ? '&' : '';
	        if (qsobj[k] != '') { //adapted, if key is empty remove from querystring
